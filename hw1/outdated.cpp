...
double ssdtotal = 0; //# ssd accesses
...
void printstatus() {						//print status of process if its not terminated, waiting or idle
	for (int i = 0; i < stats.size(); i++) {
		if (stats[i].front() != "IDLE" && stats[i].front() != "END") {
			cout << "Process " << i << " is " << stats[i].front() << endl;
		}
	}
}
void inserthdd(vector<READ> &hdd, queue<int> &HDDQueue, vector<queue<string>> &ptable, int index, int &time) {
	...
	if (hdd[0].processnum != -1) {				//push to hdd queue
		...
		stats[index].pop();				//update state
		stats[index].push("BLOCKED");
...
void updatecore(vector<coreInfo> &cores, vector<queue<string>> &ptable, queue<int> &readyQueue, int pnum, int &time) {
	...
	if (free == true) {
		...
		stats[pnum].pop();				//update state
		stats[pnum].push("RUNNING");
...
void loop(vector<coreInfo> &cores, queue<int> &readyQueue, queue<int> &SSDQueue, queue<int> &HDDQueue, priority_queue <int, vector<int>, compare> &pq, vector<READ> &ssd, vector<READ> &hdd, queue<int> &table, int index, vector<queue<string>> &ptable, int presult, int &time) {
	...
	if (presult == 1) {		//update to core
		if (ptable[index].empty()) {																		//when it terminates, print out the status of processes **************************************************************************
			...
			total++;
			stats[index].pop();
			stats[index].push("TERMINATED");
			printstatus();
			stats[index].pop();
			stats[index].push("END");
			return;
		}
...
int main() {
	...
	//organize data from file into ptable, and creates our cores
		...
		while (cin >> cmd >> value) {
			...
			if (cmd == "NCORES") {						// Set up sizes. (HEIGHT x WIDTH), first value will be process number, second value is process time
				coreNum = stoi(value);
				cores.resize(stoi(value));

				for (int i = 0; i < stoi(value); i++) {	//set default to false, to represent its not busy
					cores[i].busy = false;
					cores[i].corenum = i;
					cores[i].pnum = -1;
					cores[i].stime = 0;
			...
			else if (cmd == "NEW") {					//when a new process is found if a core is free it will occupy
				process++;
				ptable.push_back(queue<string>());			//add a queue to vector
				ptable[process].push(cmd);					//COMMAND -> VALUE is order they will be in queue, remember to take out 2 when reading from queue
				ptable[process].push(value);
				stats.push_back(queue<string>());
				stats[process].push("IDLE");
	...
	*/
	...
	printf("Average SSD acess time: %.2f ms\n", (ssdavg / ssdtotal));
